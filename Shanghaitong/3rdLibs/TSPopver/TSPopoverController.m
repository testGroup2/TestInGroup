        ...
        if (iPhone5Srceen) {
            if (IOS_VERSION >= 7.0) {
                if (SharedApp.mainTabBarViewController.tabBar.selectIndex == 0){
                    screenRect.origin.y =-1;//改变pop框的纵坐标
                }else if (SharedApp.mainTabBarViewController.tabBar.selectIndex == 1){
                    screenRect.origin.y =-8.1;
                }
            }else{
                if (SharedApp.mainTabBarViewController.tabBar.selectIndex == 0){
                    screenRect.origin.y =-1;//改变pop框的纵坐标
                }else if (SharedApp.mainTabBarViewController.tabBar.selectIndex == 1){
                    screenRect.origin.y =-9.0;
                }
            }
        }else{
            if (IOS_VERSION >= 7.0) {
                ...
                if (SharedApp.mainTabBarViewController.tabBar.selectIndex == 0){
                    ...
                    screenRect.origin.y =-8.5;
                }
            }
        }
        screenRect.size.height = screenRect.size.height-23;
        titleLabelheight = 0;
	}
	return self;
}
...
{    
    ...
    senderFrame.origin.y =  senderLocationInWindowPoint.y;//(IOS_VERSION>=7.0?62:44)
...
- (void) dismissPopoverAnimatd:(BOOL)animated
{
    if (self.view) {
        ...
        if (SharedApp.mainTabBarViewController.tabBar.selectIndex == 1) {
            NSLog(@"1111111 %@",((RViewController *) ((ResourceSliderViewController *)(((AbTabBarItem *)[SharedApp.mainTabBarViewController.tabBarItems objectAtIndex:1]).viewController)).MainVC).resouceNavbar);
...
//            }
            [self.delegate ChangeRightImageWithType:_rType];

        }else{ // 项目
            UIButton * rightButton = (UIButton *)[self.topScrollView viewWithTag:1000099];
            if (_sType == SearchHudong) {
                [rightButton setBackgroundImage:[UIImage imageNamed:@"BlueshaixuanDown.png"] forState:UIControlStateNormal];
            }else if (_sType == SearchFabu) {
                [rightButton setBackgroundImage:[UIImage imageNamed:@"YellowshaixuanDown"] forState:UIControlStateNormal];
            }else if (_sType == SearchGanxingqu) {
                [rightButton setBackgroundImage:[UIImage imageNamed:@"PinkshaixuanDown"] forState:UIControlStateNormal];
            }else if (_sType == SearchNone) {
                [rightButton setBackgroundImage:[UIImage imageNamed:@"shaixuanDown"] forState:UIControlStateNormal];
            }
        }
    }
}
